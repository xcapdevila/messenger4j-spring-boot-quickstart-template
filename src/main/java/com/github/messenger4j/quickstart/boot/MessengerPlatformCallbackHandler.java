 ...
 */
@RestController
@RequestMapping("/messenger4j/quickstart/callback")
public class MessengerPlatformCallbackHandler {
		...
		try {
			this.messenger.onReceiveEvents(payload, of(signature), event -> {
				if (event.isTextMessageEvent()) {
					...
					handlePassThreadControlEvent(event.asPassThreadControlEvent());
	...
	private void handleTextMessageEvent(TextMessageEvent event) {
		...
		logger.info("Received '{}' message '{}' with text '{}' from user '{}' at '{}'", baseEventType, messageId, messageText, senderId, timestamp);
	...
	private void sendButtonMessage(String recipientId) throws MessengerApiException, MessengerIOException, MalformedURLException {
		final List<Button> buttons = Arrays.asList(UrlButton
						.create("Open Web URL", new URL("https://www.oculus.com/en-us/rift/"), of(WebviewHeightRatio.COMPACT), of(false), empty(), empty()),
	...
	private void handleAttachmentMessageEvent(AttachmentMessageEvent event) {
		...
		for (Attachment attachment : event.attachments()) {
			if (attachment.isRichMediaAttachment()) {
				...
				String text = String.format("Media %s received (url: %s)", type.name(), url);
				text = getEventTypeBasedMessage(baseEventType, text);
				...
				String text = String.format("Location received (long: %s, lat: %s)", String.valueOf(longitude), String.valueOf(latitude));
				text = getEventTypeBasedMessage(baseEventType, text);
				sendTextMessage(senderId, text);
			}
		}
	...
	private void handleQuickReplyMessageEvent(QuickReplyMessageEvent event) {
		...
		final BaseEventType baseEventType = event.baseEventType();
		logger.debug("baseEventType: {}", baseEventType);
		logger.info("Received '{}' quick reply for message '{}' with payload '{}'", baseEventType, messageId, payload);
		String text = "Quick reply tapped";
		text = getEventTypeBasedMessage(baseEventType, text);
		sendTextMessage(senderId, text);
	}
	...
	private void handlePostbackEvent(PostbackEvent event) {
		...
		final BaseEventType baseEventType = event.baseEventType();
		logger.debug("baseEventType: {}", baseEventType);
		logger.info("Received '{}' postback for user '{}' and page '{}' with payload '{}' at '{}'", baseEventType, senderId, senderId, payload, timestamp);
		String text = "Postback event tapped";
		text = getEventTypeBasedMessage(baseEventType, text);
		sendTextMessage(senderId, text);
	}
	...
	private void handleAccountLinkingEvent(AccountLinkingEvent event) {
		...
		logger.info("Received '{}' account linking event for user '{}' with status '{}' and auth code '{}'", baseEventType, senderId, accountLinkingStatus,
				authorizationCode);
		String text = "AccountLinking event tapped";
		text = getEventTypeBasedMessage(baseEventType, text);
		sendTextMessage(senderId, text);
	}
	...
	private void handleOptInEvent(OptInEvent event) {
		...
		logger.info("Received '{}' authentication for user '{}' and page '{}' with pass through param '{}' at '{}'", baseEventType, senderId, recipientId,
				passThroughParam, timestamp);
	...
	private void handleMessageEchoEvent(MessageEchoEvent event) {
		...
		logger.info("Received '{}' echo for message '{}' that has been sent to recipient '{}' by sender '{}' at '{}'", baseEventType, messageId, recipientId,
				senderId, timestamp);
		String text = "MessageEchoEvent tapped";
		text = getEventTypeBasedMessage(baseEventType, text);
		sendTextMessage(senderId, text);
	}
	...
	private void handleMessageDeliveredEvent(MessageDeliveredEvent event) {
		...
		final String eventTypeBasedMessage = getEventTypeBasedMessage(baseEventType, "Received delivery confirmation for message '{}'");
		messageIds.forEach(messageId -> {
			logger.info(eventTypeBasedMessage, messageId);
		});
	...
	private void handleMessageReadEvent(MessageReadEvent event) {
		...
		logger.info(getEventTypeBasedMessage(baseEventType, "All messages before '{}' were read by user '{}'"), watermark, senderId);
	}
	...
	private void handleFallbackEvent(Event event) {
		...
		logger.info(getEventTypeBasedMessage(baseEventType, "Received unsupported message from user '{}'"), senderId);
	}

	private void handlePassThreadControlEvent(PassThreadControlEvent event) {
		logger.debug("Handling PassThreadControlEvent");
		final String senderId = event.senderId();
		logger.debug("senderId: {}", senderId);
		final BaseEventType baseEventType = event.baseEventType();
		logger.debug("baseEventType: {}", baseEventType);
	...
	private void handleRequestThreadControlEvent(RequestThreadControlEvent event) {
		logger.debug("Handling RequestThreadControlEvent");
		final String senderId = event.senderId();
		logger.debug("senderId: {}", senderId);
		final String recipientId = event.recipientId();
		logger.debug("recipientId: {}", recipientId);
	...
	private void handleTakeThreadControlEvent(TakeThreadControlEvent event) {
		logger.debug("Handling TakeThreadControlEvent");
		final String senderId = event.senderId();
		logger.debug("senderId: {}", senderId);
		final String recipientId = event.recipientId();
		logger.debug("recipientId: {}", recipientId);
